...
def evaluate(model, criterion, postprocessors, data_loader, base_ds, device, output_dir, wo_class_error=False, args=None, logger=None):
    savenet = torch.nn.Linear(2, 2)
    savenet_dict = savenet.state_dict()
    try:
        need_tgt_for_training = args.use_dn
    except:
        need_tgt_for_training = False
    ...
    # for i, layer in enumerate(model.transformer.decoder.layers.children()):
    ...
    for samples, targets in metric_logger.log_every(data_loader, 10, header, logger=logger):
        ...
        boxes = box_ops.box_xyxy_to_cxcywh(results[0]['boxes'])
        select_mask = scores > thershold
        # print(targets)
       

        res_dict[int(targets[0]['image_id'].cpu().detach().numpy())] = {
          'image_id': targets[0]['image_id'],
          'size': targets[0]['size'],
          'box_label': targets[0]['labels'],
          'pred_boxes': boxes[select_mask],
          'norm_pred_boxes': outputs['pred_boxes'],
        }
    ...
    if args.save_results:
      savenet_dict.update(res_dict)
      torch.save(savenet_dict, "t5_gt_preds.pth")
